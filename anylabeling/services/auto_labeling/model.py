...
from anylabeling.paths import get_data_root
...
class Model(QObject):
    ...
    def allow_migrate_data():
        ...
        new_model_path = str(get_data_root())
        ...
        new_model_parent = os.path.dirname(new_model_path)
        if not os.access(new_model_parent, os.W_OK):
            return False
    ...
    def get_model_abs_path(self, model_config, model_path_field_name):
        ...
        model_path = str(get_data_root())
        if not migrate_flag:
            logger.warning(
                "Data migration check failed, using project data directory."
            )
        model_abs_path = os.path.abspath(
            os.path.join(
                model_path,
