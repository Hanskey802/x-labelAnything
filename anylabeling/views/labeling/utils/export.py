...
class CommandThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.finished.emit(completed.returncode == 0, output, "")
        except Exception as e:
            self.finished.emit(False, "", str(e))
...
def _guess_json_to_yolo_input_root(self):
    ...
    current_path = pathlib.Path(self.filename).resolve()
    parent = current_path.parent
    if parent.name in {"train", "val", "test", "raw", "sliced"}:
        images_dir = parent.parent
        if images_dir.name == "images":
            return str(images_dir.parent)
    if parent.name == "images":
        return str(parent.parent)
    return str(parent)


def _resolve_json_to_yolo_script_path():
...
def export_xanylabeling_json_to_yolo(self):
    script_path = _resolve_json_to_yolo_script_path()
    ...
    default_write_empty = self.settings.value(
        "json_to_yolo/write_empty", True, type=bool
    )

    dialog = QtWidgets.QDialog(self)
    dialog.setWindowTitle(self.tr("批量转换 X-AnyLabeling JSON 到 YOLO"))
    ...
    intro_label = QtWidgets.QLabel(
        self.tr(
            "将 X-AnyLabeling 的矩形框 JSON 标注批量转换为 "
            "YOLO 检测标签。"
        )
    )
    ...
    def create_path_row(label_text, default_text, browse_callback):
        ...
        browse_button.setStyleSheet(get_cancel_btn_style())
    ...
    def browse_yaml(target_edit):
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self,
            self.tr("选择 YAML 文件"),
            target_edit.text() or ".",
            self.tr("YAML 文件 (*.yaml *.yml);;所有文件 (*)"),
        )
        if path:
            target_edit.setText(path)

    input_layout, input_edit = create_path_row(
        self.tr("输入根目录"),
        default_input_root,
        browse_dir,
    )
    input_edit.setPlaceholderText(
        self.tr("包含 images/train 和 json 文件的数据集根目录")
    )
    layout.addLayout(input_layout)

    output_layout, output_edit = create_path_row(
        self.tr("输出根目录"),
        default_output_root,
        browse_dir,
    )
    output_edit.setPlaceholderText(
        self.tr("YOLO 数据集输出位置")
    )
    layout.addLayout(output_layout)

    classes_layout, classes_edit = create_path_row(
        self.tr("类别 YAML（可选）"),
        default_classes_yaml,
        browse_yaml,
    )
    classes_edit.setPlaceholderText(
        self.tr("可选，填写包含 classes 列表的模型 yaml")
    )
    layout.addLayout(classes_layout)

    write_empty_checkbox = QtWidgets.QCheckBox(
        self.tr("为没有目标的图片生成空的 txt 标注文件")
    )
    write_empty_checkbox.setChecked(default_write_empty)
    layout.addWidget(write_empty_checkbox)

    script_label = QtWidgets.QLabel(
        self.tr("脚本位置：{}").format(str(script_path))
    )
    ...
    ok_button = QtWidgets.QPushButton(self.tr("开始转换"))
    ...
    input_root = input_edit.text().strip()
    output_root = output_edit.text().strip()
    classes_yaml = classes_edit.text().strip()
    ...
    if not input_root:
        ...
        return
    if not pathlib.Path(input_root).exists():
        QtWidgets.QMessageBox.warning(
            self,
            self.tr("输入目录无效"),
            self.tr("输入根目录不存在：\n{}").format(input_root),
    ...
    if not output_root:
        ...
        return
    if classes_yaml and not pathlib.Path(classes_yaml).exists():
        QtWidgets.QMessageBox.warning(
            self,
            self.tr("YAML 无效"),
            self.tr("类别 YAML 不存在：\n{}").format(classes_yaml),
    ...
    self.settings.setValue("json_to_yolo/input_root", input_root)
    self.settings.setValue("json_to_yolo/output_root", output_root)
    self.settings.setValue("json_to_yolo/classes_yaml", classes_yaml)
    self.settings.setValue(
        "json_to_yolo/write_empty", write_empty_checkbox.isChecked()
    )

    command = [sys.executable, str(script_path), "--input-root", input_root]
    command.extend(["--output-root", output_root])
    if classes_yaml:
        command.extend(["--classes-yaml", classes_yaml])
    if write_empty_checkbox.isChecked():
        command.append("--write-empty")

    progress = QProgressDialog(
        self.tr("正在把 JSON 标注转换为 YOLO，请稍候..."),
        None,
        0,
        0,
        self,
    )
    progress.setWindowTitle(self.tr("请稍候"))
    progress.setWindowModality(Qt.WindowModal)
    progress.setCancelButton(None)
    progress.setMinimumDuration(0)
    progress.show()

    worker = CommandThread(command, workdir=str(script_path.parent))
    self._json_to_yolo_worker = worker

    def handle_finished(success, output, error_message):
        progress.close()
        self._json_to_yolo_worker = None

        details = output.strip() if output else error_message.strip()
        if success:
            QtWidgets.QMessageBox.information(
                self,
                self.tr("转换完成"),
                self.tr("YOLO 数据集已生成到：\n{}").format(output_root)
